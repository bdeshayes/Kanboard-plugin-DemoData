...
class DemoData extends BaseController {
	...
	{ 
	$blurb = array('Customisation', 'Transition', "Expectation", 'Investigation', 'Cogitation', 'Regression', "Degustation",
	'Delegation', 'Compilation', 	'Manipulation', 'Transmission', 'Evaluation', 'Situation', 'Revolution', 'Imagination', 'Machination', 'Ambition');
	return array_rand(array_flip($blurb), 1); 
	}
	...
	if ($this->request->isPost()) 
		...
		$this->doImport($phu, "project_has_users");
		...
		$this->doImport($phg, "project_has_groups");
		...
		for ($i=1; $i<6; $i++)
			{
			array_push ($links, array('link_id' => 6, 'task_id' => $i, 'opposite_task_id' => 21 - $i));
			array_push ($links, array('link_id' => 7, 'task_id' => 21 - $i, 'opposite_task_id' => $i));
			}
